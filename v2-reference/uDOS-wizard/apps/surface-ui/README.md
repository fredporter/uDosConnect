...
## Production path (Wizard `/app`)

`npm run build` emits asset URLs under **`/app-assets/`** so the FastAPI app can mount `dist/` at that prefix (`wizard/main.py`). If `/app` is blank in the browser, rebuild after pulling and hard-refresh.
