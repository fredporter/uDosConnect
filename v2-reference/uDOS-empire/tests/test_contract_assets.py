...
from udos_empire_compat.legacy_sync_runtime import (
    ...
    probe_local_host_automation_app,
    probe_local_host_automation_cycle,
    probe_local_host_automation_runtime,
    probe_local_host_app,
...
class ContractAssetTests(unittest.TestCase):
    def test_sync_adapter_attaches_transport_targets(self) -> None:
        ...
        self.assertTrue(plan["sync_record_profile_source"].endswith("uDOS-empire/src/sync-record-profile.json"))
        self.assertTrue(plan["sync_record_example_source"].endswith("uDOS-empire/examples/basic-sync-record-envelope.json"))
        self.assertTrue(plan["container_job_catalog_source"].endswith("uDOS-empire/src/containers/container-job-catalog.json"))
    ...
    def test_sync_record_profile_aligns_to_shared_core_contract(self) -> None:
        ...
        self.assertEqual(payload["owner"], "uDOS-empire")
    ...
    def test_dispatch_sync_package_posts_payload_to_host_runtime(self) -> None:
        plan = build_sync_plan(REPO_ROOT, channel_name="hubspot-sync")
        dispatched = dispatch_sync_package(
            plan,
            host_url="http://ubuntu.local",
        ...
        self.assertEqual(handoff["target"], "http://ubuntu.local/api/runtime/sync-records/ingest")
    ...
    def test_build_automation_jobs_for_channel_specific_package(self) -> None:
        ...
        self.assertEqual(job["requested_capability"], "repo.host.job.hubspot-sync")
        self.assertEqual(job["policy_flags"]["channel"], "hubspot-sync")
        self.assertEqual(job["policy_flags"]["container_job_id"], "hubspot-sync")
        self.assertEqual(job["policy_flags"]["host_action_id"], "repo.host.job.hubspot-sync")

    def test_dispatch_automation_jobs_posts_to_host_runtime(self) -> None:
        ...
        self.assertEqual(handoff["target"], "http://ubuntu.local/api/runtime/automation/jobs")
        self.assertEqual(handoff["job_count"], 1)
        self.assertEqual(handoff["responses"][0]["requested_capability"], "repo.host.job.hubspot-sync")
    ...
    def test_dispatch_automation_results_posts_to_host_runtime(self) -> None:
        plan = build_automation_results(
            build_automation_jobs(build_sync_package(build_sync_plan(REPO_ROOT, channel_name="hubspot-sync")))
        )
        ...
        self.assertEqual(handoff["target"], "http://ubuntu.local/api/runtime/automation/results")
        self.assertEqual(handoff["result_count"], 1)
        self.assertEqual(handoff["responses"][0]["status"], "completed")

    def test_process_automation_jobs_posts_to_host_runtime(self) -> None:
        plan = build_automation_jobs(build_sync_package(build_sync_plan(REPO_ROOT, channel_name="hubspot-sync")))
        ...
        self.assertEqual(handoff["target"], "http://ubuntu.local/api/runtime/automation/process-next")
    ...
    def test_fetch_automation_results_reads_runtime_state(self) -> None:
        ...
        self.assertEqual(state["target"], "http://ubuntu.local/api/runtime/automation/results")
    ...
    def test_build_automation_runtime_summary_maps_status_and_channel(self) -> None:
        ...
        plan["automation_results_state"] = {
            "target": "http://ubuntu.local/api/runtime/automation/results",
            "payload": {
                "contract_version": "v2.0.4",
                "items": [
                    {
    ...
    def test_probe_local_host_app_ingests_sync_package(self) -> None:
        plan = build_sync_package(build_sync_plan(REPO_ROOT))
        probed = probe_local_host_app(plan, workspace_root=REPO_ROOT.parent)
        self.assertEqual(probed["local_sync_handoff"]["status_code"], 200)
        self.assertTrue(probed["local_sync_handoff"]["payload"]["ok"])

    def test_probe_local_host_automation_app_queues_jobs(self) -> None:
        plan = build_automation_jobs(build_sync_package(build_sync_plan(REPO_ROOT, channel_name="hubspot-sync")))
        probed = probe_local_host_automation_app(plan, workspace_root=REPO_ROOT.parent)
        ...
        self.assertEqual(
            ...
            "repo.host.job.hubspot-sync",
        )

    def test_probe_local_host_automation_cycle_records_results(self) -> None:
        plan = build_automation_results(
            build_automation_jobs(build_sync_package(build_sync_plan(REPO_ROOT, channel_name="hubspot-sync")))
        )
        probed = probe_local_host_automation_cycle(plan, workspace_root=REPO_ROOT.parent)
    ...
    def test_probe_local_host_automation_runtime_processes_jobs(self) -> None:
        plan = build_automation_jobs(build_sync_package(build_sync_plan(REPO_ROOT, channel_name="hubspot-sync")))
        probed = probe_local_host_automation_runtime(plan, workspace_root=REPO_ROOT.parent)
    ...
    def test_probe_local_uhome_aliases_still_work(self) -> None:
        plan = build_sync_package(build_sync_plan(REPO_ROOT))
    ...
    def test_webhook_templates_have_expected_keys(self) -> None:
        targets = [
            ...
            REPO_ROOT / "src" / "webhooks" / "google-mvp-binder-trigger-template.json",
    ...
    def test_google_mvp_binder_trigger_assets_define_mirror_only_lane(self) -> None:
        template = json.loads(
            (REPO_ROOT / "src" / "webhooks" / "google-mvp-binder-trigger-template.json").read_text(encoding="utf-8")
        )
        example = json.loads(
            (REPO_ROOT / "examples" / "google-mvp-binder-trigger.json").read_text(encoding="utf-8")
        )
        self.assertEqual(template["lane_id"], "google-mvp-a")
        self.assertEqual(template["mirror_rules"]["remote_role"], "firestore-mirror")
        self.assertEqual(template["mirror_rules"]["canonical_truth"], "vault")
        self.assertTrue(example["artifact"]["approved_for_mirror"])
        self.assertEqual(example["target"]["remote_role"], "firestore-mirror")
        self.assertIn("route list", template["extraction_required"])

    def test_google_mvp_generated_service_extract_matches_binder_trigger_lane(self) -> None:
        payload = json.loads(
            (REPO_ROOT / "examples" / "google-mvp-generated-service-extract.json").read_text(encoding="utf-8")
        )
        self.assertEqual(payload["lane_id"], "google-mvp-a")
        self.assertEqual(payload["service"]["name"], "google-mvp-binder-trigger")
        self.assertEqual(payload["service"]["remote_role"], "firestore-mirror")
        self.assertEqual(payload["routes"][0]["path"], "/binder/google-mvp/mirror")
        self.assertIn("BINDER_TRIGGER_SECRET", payload["environment_variables"])
    ...
    def test_container_job_catalog_is_empire_owned(self) -> None:
        ...
        self.assertEqual(payload["owner"], "uDOS-empire")
        self.assertEqual(payload["runtime_owner"], "uDOS-host")
        self.assertEqual(payload["canonical_runtime_owner"], "uDOS-host")
        self.assertTrue(payload["jobs"][0]["host_action_id"].startswith("repo.host.job."))
