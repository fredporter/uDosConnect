...
class PackRuntimeTests(unittest.TestCase):
    def test_list_packs_returns_starter_packs(self) -> None:
        ...
        self.assertIn("wordpress-contact-import", pack_ids)
    ...
    def test_build_pack_catalog_reports_counts(self) -> None:
        ...
        self.assertIn("legacy-transition", catalog["status_counts"])
        self.assertIn("current", catalog["status_counts"])
    ...
    def test_render_pack_preview_renders_campaign_starter(self) -> None:
        ...
        self.assertEqual(preview["status"], "legacy-transition")
    ...
    def test_render_pack_preview_renders_wordpress_contact_import(self) -> None:
        preview = render_pack_preview(REPO_ROOT, "wordpress-contact-import")
        self.assertEqual(preview["pack_id"], "wordpress-contact-import")
        self.assertEqual(preview["status"], "current")
        self.assertEqual(preview["preview_status"], "ready")
        self.assertIn("wordpress-plugin", preview["touched_systems"])
        self.assertIn("brisbane-launch-leads-01", preview["rendered_outputs"][0]["rendered"])
    ...
    def test_build_pack_handoff_summary_reports_none_for_wordpress_contact_import(self) -> None:
        handoff = build_pack_handoff_summary(REPO_ROOT, "wordpress-contact-import")
        self.assertEqual(handoff["handoff_kind"], "none")
        self.assertEqual(handoff["channel"], None)
    ...
    def test_build_pack_execution_brief_reports_next_step(self) -> None:
        ...
        self.assertEqual(brief["status"], "legacy-transition")
    ...
    def test_dispatch_pack_posts_sync_package(self) -> None:
        ...
        self.assertEqual(
            ...
            "http://ubuntu.local/api/runtime/sync-records/ingest",
        )
    ...
    def test_dispatch_pack_processes_automation_lane(self) -> None:
        ...
        self.assertEqual(
            ...
            "http://ubuntu.local/api/runtime/automation/jobs",
        )
        self.assertEqual(
            dispatched["automation_process_handoff"]["target"],
            "http://ubuntu.local/api/runtime/automation/process-next",
        )
    ...
    def test_pack_run_smoke_can_dispatch_to_local_host_app(self) -> None:
        proc = subprocess.run(
            [
                sys.executable,
