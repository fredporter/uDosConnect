...
from udos_empire.packs import default_artifact_path, dispatch_pack, probe_local_pack_dispatch, write_pack_run_report


def main() -> int:
    parser = argparse.ArgumentParser(description="Dispatch a uDOS-empire starter pack")
    parser.add_argument("--pack", default="campaign-starter", help="Pack id to dispatch")
    parser.add_argument("--host-url", help="Optional Ubuntu host compatibility base URL")
    parser.add_argument("--local-host-app", action="store_true", help="Use in-process sibling Ubuntu host compatibility app")
    parser.add_argument("--uhome-url", help=argparse.SUPPRESS)
    ...
    host_url = args.host_url or args.uhome_url
    local_host_app = args.local_host_app or args.local_uhome_app
    if bool(host_url) == bool(local_host_app):
        raise SystemExit("choose exactly one of --host-url or --local-host-app")

    if local_host_app:
        payload = probe_local_pack_dispatch(REPO_ROOT, args.pack, workspace_root=REPO_ROOT.parent)
    else:
        payload = dispatch_pack(REPO_ROOT, args.pack, host_url, process_automation=args.process_automation)
