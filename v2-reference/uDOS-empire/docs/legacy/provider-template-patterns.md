# uDOS-empire Provider Template Patterns

## Purpose

Capture reusable provider-template shapes from the archived Empire webhook
templates and adapt them for the v2 `uDOS-empire` contract lane.
...
That pattern still fits the new `uDOS-empire` webhook lane even though the old
