...
def build_pack_handoff_summary(repo_root: Path, pack_id: str) -> dict[str, Any]:
    ...
    if "hubspot-sync" in container_jobs:
        from udos_empire_compat.legacy_sync_runtime import build_sync_package, build_sync_plan
    ...
    if "binder-release-webhook" in container_jobs:
        from udos_empire_compat.legacy_sync_runtime import build_automation_jobs, build_sync_package, build_sync_plan
...
def dispatch_pack(
    ...
    if handoff["handoff_kind"] == "sync-package":
        from udos_empire_compat.legacy_sync_runtime import build_sync_package, build_sync_plan, dispatch_sync_package

        plan = build_sync_package(build_sync_plan(repo_root, channel_name=handoff["channel"]))
        dispatched = dispatch_sync_package(plan, host_url, poster=poster)
        return {
            "pack_id": pack_id,
            "dispatch_mode": "http",
            "handoff_kind": "sync-package",
    ...
    if handoff["handoff_kind"] == "automation-job":
        ...
        dispatched = dispatch_automation_jobs(plan, host_url, poster=poster)
        result: dict[str, Any] = {
            "pack_id": pack_id,
            "dispatch_mode": "http",
            "handoff_kind": "automation-job",
        ...
        if process_automation:
            processed = process_automation_jobs(plan, host_url, poster=poster)
            fetched = fetch_automation_results(plan, host_url, fetcher=fetcher)
...
def probe_local_pack_dispatch(
    ...
    if handoff["handoff_kind"] == "sync-package":
        from udos_empire_compat.legacy_sync_runtime import build_sync_package, build_sync_plan, probe_local_host_app

        plan = build_sync_package(build_sync_plan(repo_root, channel_name=handoff["channel"]))
        probed = probe_local_host_app(plan, workspace_root=workspace_root)
        return {
            "pack_id": pack_id,
    ...
    if handoff["handoff_kind"] == "automation-job":
        ...
        plan = build_automation_jobs(build_sync_package(build_sync_plan(repo_root, channel_name=handoff["channel"])))
        probed = probe_local_host_automation_runtime(plan, workspace_root=workspace_root)
        return {
            "pack_id": pack_id,
            "dispatch_mode": "local-app",
            "handoff_kind": "automation-job",
