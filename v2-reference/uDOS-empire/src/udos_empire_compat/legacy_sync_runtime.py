...
def _channel_host_action_id(channel_name: str) -> str:
    mapping = {
        "google-workspace-mirror": "repo.host.job.google-workspace-sync",
        "hubspot-sync": "repo.host.job.hubspot-sync",
        "webhook-automation": "repo.host.job.binder-release-webhook",
    }
    return mapping.get(channel_name, "repo.host.job.unknown")
...
def _resolve_host_repo(workspace_root: Path) -> tuple[Path, str]:
    ubuntu_repo = workspace_root / "uDOS-host"
    if ubuntu_repo.is_dir():
        return ubuntu_repo, "uDOS-host"
    legacy_repo = workspace_root / "uHOME-server"
    return legacy_repo, "uHOME-server"


def _build_internal_host_compat_app():
    ...
    @app.post("/api/runtime/sync-records/ingest")
    def ingest_sync_records(payload: dict) -> dict:
        sync_items.append(payload)
        return {
            "ok": True,
            "ingested": True,
            "counts": {
                ...
                "sync_metadata": len(payload.get("sync_metadata", [])),
            },
        }

    @app.post("/api/runtime/automation/jobs")
    def queue_automation_job(payload: dict) -> dict:
        automation_jobs.append(payload)
        return {
            "ok": True,
    ...
    @app.post("/api/runtime/automation/process-next")
    def process_next(_: Optional[dict] = None) -> dict:
        if not automation_jobs:
            return {"ok": True, "status": "idle"}
        job = automation_jobs.pop(0)
        result = {
        ...
        return {"ok": True, "status": "processed", "job": job, "result": result}

    @app.post("/api/runtime/automation/results")
    def record_result(payload: dict) -> dict:
        automation_results.append(payload)
        return {
            "ok": True,
            "job_id": payload["job_id"],
    ...
    @app.get("/api/runtime/automation/results")
    def list_results() -> dict:
        return {
            "contract_version": "v2.0.4",
            "items": automation_results,
        }

    @app.get("/api/runtime/automation/status")
    def automation_status() -> dict:
        ...
        return {
            "job_count": len(automation_jobs),
            "result_count": len(automation_results),
...
def _load_host_runtime_app(workspace_root: Path):
    server_repo, host_repo_name = _resolve_host_repo(workspace_root)
    candidate = server_repo / "src"
    if candidate.is_dir():
        sys.path.insert(0, str(candidate))
    try:
        from uhome_server.app import create_app  # type: ignore

        return create_app, host_repo_name
    except ModuleNotFoundError:
        return _build_internal_host_compat_app, f"{host_repo_name}-compat"
    ...
    services = [
        {
            ...
            "consumer": "uDOS-empire",
            "usage": _usage_for_service(service["key"]),
        }
        for service in manifest["services"]
        if "uDOS-empire" in service.get("consumers", [])
    ]
...
def build_automation_jobs(plan: dict) -> dict:
    ...
    host_action_ids = {
        item["service"]: item.get("host_action_id", _channel_host_action_id(item["service"]))
        for item in plan.get("container_jobs", [])
    }
    queued_at = _utc_now_iso_z()
    jobs = []
    for channel_name in package["channels"]:
        requested_capability = host_action_ids.get(channel_name, _channel_host_action_id(channel_name))
        jobs.append(
            {
    ...
    enriched["automation_job_summary"] = {
        ...
        "requested_capability": jobs[0]["requested_capability"] if jobs else None,
    }
    return enriched
        ...
        results.append(
            {
                ...
                "event_refs": [f"event://uDOS-empire/automation/{job['job_id']}"],
...
def _resolve_host_url(host_url: str | None = None, uhome_url: str | None = None) -> str:
    resolved = host_url or uhome_url
    if not resolved:
...
def dispatch_sync_package(
    ...
    *,
    uhome_url: str | None = None,
) -> dict:
    post = poster or _default_poster
    package = plan.get("sync_package") or build_sync_package(plan)["sync_package"]
    payload = package["payload"]
    host_url = _resolve_host_url(host_url, uhome_url)
    target = f"{host_url}/api/runtime/sync-records/ingest"
...
def dispatch_automation_jobs(
    ...
    *,
    ...
    jobs = plan.get("automation_jobs") or build_automation_jobs(plan)["automation_jobs"]
    host_url = _resolve_host_url(host_url, uhome_url)
    target = f"{host_url}/api/runtime/automation/jobs"
...
def process_automation_jobs(
    ...
    *,
    ...
    jobs = plan.get("automation_jobs") or build_automation_jobs(plan)["automation_jobs"]
    host_url = _resolve_host_url(host_url, uhome_url)
    target = f"{host_url}/api/runtime/automation/process-next"
...
def dispatch_automation_results(
    ...
    *,
    ...
    host_url = _resolve_host_url(host_url, uhome_url)
    target = f"{host_url}/api/runtime/automation/results"
...
def build_automation_runtime_summary(plan: dict) -> dict:
    ...
    local_runtime = plan.get("local_automation_runtime", {})
    processed_payloads = local_runtime.get("processed_payloads", [])
    local_processed_items = [
        payload["result"]
        for payload in processed_payloads
        if isinstance(payload, dict) and isinstance(payload.get("result"), dict)
    ]
    if local_processed_items:
        result_items = local_processed_items
    if not result_items:
        result_items = local_runtime.get("results_payload", {}).get("items")
    if not result_items:
        result_items = plan.get("automation_results") or []
    if job_map:
        filtered_items = [item for item in result_items if item.get("job_id") in job_map]
        if filtered_items:
    ...
    if counts_by_channel.get("unknown") and len(set(job_map.values())) == 1:
        channel_name = next(iter(job_map.values()))
        counts_by_channel[channel_name] = counts_by_channel.get(channel_name, 0) + counts_by_channel.pop("unknown")
...
def probe_local_host_app(plan: dict, workspace_root: Path) -> dict:
    from fastapi.testclient import TestClient

    create_app, host_repo_name = _load_host_runtime_app(workspace_root)
...
def probe_local_host_automation_app(plan: dict, workspace_root: Path) -> dict:
    from fastapi.testclient import TestClient
...
def probe_local_host_automation_cycle(plan: dict, workspace_root: Path) -> dict:
    from fastapi.testclient import TestClient
...
def probe_local_host_automation_runtime(plan: dict, workspace_root: Path) -> dict:
    from fastapi.testclient import TestClient
...
def probe_local_uhome_app(plan: dict, workspace_root: Path) -> dict:
    return probe_local_host_app(plan, workspace_root)


def probe_local_uhome_automation_app(plan: dict, workspace_root: Path) -> dict:
    return probe_local_host_automation_app(plan, workspace_root)


def probe_local_uhome_automation_cycle(plan: dict, workspace_root: Path) -> dict:
    return probe_local_host_automation_cycle(plan, workspace_root)


def probe_local_uhome_automation_runtime(plan: dict, workspace_root: Path) -> dict:
    return probe_local_host_automation_runtime(plan, workspace_root)
